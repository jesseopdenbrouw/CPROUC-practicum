...
\lstset{
  keywords=[10]{fib,leesCijfer,fflush,exit,malloc,free,realloc,catalan,faculteit,catalanRecursief,collatzSteps,max2,leesGetal,gemiddelde,kgv,ggd,leesEnBerekenSom,leesEnBerekenMod9,isprime,isisolatedprime,sinc,aantal_delers,reverse,strrev,is_palindroom,controleerNedEuroID,doorsnede,uniek,rlecompress,roll_dice,rand,stringreplace,scanf_s,getoneline},  % User functions
  keywords=[11]{punt},  % Typedef'd types
  keywords=[12]{},  % Macros
...
Schrijf een programma dat de tafel van 12 afdrukt. Maar gebruik van een \lstinline|for|-statement voor het opzetten van de herhaling.
...
Schrijf een programma dat een positief geheel getal inleest en vervolgens de driehoek van Floyd afdrukt. In deze driehoek worden getallen oplopend afgedrukt, beginnend bij 1, steeds één getal meer per rij. Een voorbeeld van de driehoek voor invoer van het getal 6 is hieronder gegeven.
...
\subsection{Oppervlakte bepalen}[D]
...
\begin{tikzpicture}
    ...
    \node at (axis cs: 2.5,-0.7) {$O$};
    \draw[-Circle] (axis cs: 2.5,-0.6) -- (axis cs: 3,1);
    \end{axis}
\end{tikzpicture}%
\end{figure}

Schrijf een programma dat de oppervlakte van de functie berekent binnen het domein $x=0$ tot en met $x=5$. Dit is in de figuur aangegeven met de lichtblauwe kleur. De oppervlakte is aangegeven met de letter \textsl{O}. Tip: lees het boek. Overigens is het antwoord exact $125/24 \approx 5,2083\cdots$
...
\subsection{Inleiding}
Een structure in een handige manier om bij elkaar behorende gegevens te gebruiken onder één gemeenschappelijke noemer. Het voordeel van een structure is dat het als geheel kan worden meegegeven als een argument in een functie en dat een functie een complete structure kan retourneren. We hoeven ons dat niet druk te maken over wat er precies in de structure zit.

Als voorbeeld zie je hier een structure die twee doubles bevat. De structure heeft als naam \lstinline|punt|.
...
We kunnen nu twee variabelen van de structure aanmaken:

\begin{lstlisting}[caption=]
struct punt A, B;
\end{lstlisting}

We kunnen een variabele gelijk initialiseren:

\begin{lstlisting}[caption=]
struct punt C = { 1.2, 3.4 };
\end{lstlisting}

Maar we kunnen ook de \textsl{members} apart een waarde toekennen:
...
Schrijf een programma dat een array van tien integers met behulp van een pointer afdrukt. Definieer zelf een array.

\subsection{Array afdrukken, reprise}[B]
Schrijf een programma dat een array van \textsl{onbekende} lengte gevuld met integers met behulp van een pointer afdrukt. Definieer zelf een array.

%\subsection{Array van string}[A]
%Schrijf een programma dat een array van strings afdruk.
