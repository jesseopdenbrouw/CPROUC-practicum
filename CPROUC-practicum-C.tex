...
%%
%%   (c) 2023  --  The Hague University of Applied Sciences (THUAS)
...
%  belowskip=-2.2ex,
  ...
  alsoletter={.},
  keywords=[4]{assert.h,complex.h,ctype.h,errno.h,fenv.h,float.h,inttypes.h,iso646.h,limits.h,locale.h,malloc.h,math.h,setjmp.h,signal.h,stdalign.h,stdarg.h,stdatomic.h,stdbool.h,stddef.h,stdint.h,stdio.h,stdlib.h,stdnoreturn.h,string.h,tgmath.h,threads.h,time.h,uchar.h,wchar.h,wctype.h},
...
\lstset{
  keywords=[10]{fib,leesCijfer,fflush,exit,malloc,free,realloc,catalan,faculteit,catalanRecursief,collatzSteps,max2,leesGetal,gemiddelde,kgv,ggd,leesEnBerekenSom,leesEnBerekenMod9,isprime,isisolatedprime,sinc,aantal_delers,reverse,strrev,is_palindroom,controleerNedEuroID,doorsnede,uniek,rlecompress,roll_dice,rand,stringreplace,scanf_s,getoneline,opvolgorde,init,get,put,maximum},  % User functions
  keywords=[11]{punt,student_struct},  % Typedef'd types
  keywords=[12]{},  % Macros
...
\newtcblisting{dosbox}[1][]{%
  ...
  listing options={frame=none,aboveskip=6pt,belowskip=0pt,numbers=none},
...
Test het programma met de volgende invoer.

Voer \lstinline|0| in, uitvoer is \lstinline|1|\\
Voer \lstinline|5| in, uitvoer is \lstinline|42|\\
Voer \lstinline|7| in, uitvoer is \lstinline|429|\\
Voer \lstinline|9| in, uitvoer is \lstinline|4862|\\
Voer \lstinline|10| in, uitvoer is \lstinline|16796|
...
Let erop dat de queue vol kan zijn, dus een aanroep met \lstinline|put| moet dan een foutmelding geven. Let erop dat de queue leeg kan zijn, dus een aanroep met \lstinline|get| moet dan een foutmelding geven.
...
struct student_struct {
	int idcode;           // An integer
	char achternaam[30];  // A string of 29 characters
    char klas[10];        // A string of 9 characters
	double resultaat;     // A double between 0.0 and 10.0
};

struct student_struct jan;

jan.idcode = 21011009;                 // an integer
strcpy(jan.achternaam, "Klaassen");    // no more than 29 characters!
strcpy(jan.klas, "ep13");              // no more than 9 characters!
jan.resultaat = 7.3;                   // a double between 0.0 and 10.0

printf("%8d %-19s %-9s %4.1f\n", jan.idcode, jan.achternaam, jan.klas, jan.resultaat);
\end{lstlisting}

Je kan de \textsl{members} van de structure (\lstinline|idcode|, \lstinline|achternaam|, \lstinline|resultaat|) gewoon gebruiken als andere variabelen. Dit is te zien in de toekenningen en in de \lstinline|printf|-regel.

Het is ook mogelijk om een \textsl{array van structures} te definiëren, bijvoorbeeld alle studenten in een klas. We gebruiken hiervoor weer \lstinline|struct student_struct| maar geven nu een array op. Met behulp van een \lstinline|for|-lus kan je langs alle elementen uit de de array ``lopen''.

\begin{lstlisting}[caption=]
struct student_struct klas[50];
...
\subsection{Tijdsverschil}
Schrijf een programma dat twee tijden inleest (in uren, minuten en seconden) en het verschil tussen deze tijden (in seconden) afdrukt. Gebruik de structure uit opdracht~\ref{opd:structtijd}. 
 
\subsection{Studenten}[B]
\label{sec:struct_stud}
In de inleiding (paragraaf~\ref{sec:struct_inleiding}) wordt de structure \lstinline|student_struct| beschreven. Hiermee kunnen we gegeven van studenten en het resultaat van een toets opslaan. Gebruik deze structure.

Schrijf een programma dat de gegevens van 5 van deze studenten inleest en afdrukt. Gebruik hiervoor een functie \lstinline|lees_student| en \lstinline|print_student|. Gebruik Call by Value voor argument/parameter-overdracht en returnwaarde. Bij het afdrukken moeten de gegevens netjes onder elkaar worden afgedrukt, alsof het in kolommen is afgedrukt.


\subsection{Studenten again}[c]
Als opdracht~\ref{sec:struct_stud} maar realiseer nu ook een functie \lstinline|maximum| die het maximum behaalde resultaat retourneert. Gebruik de functie in een array van structures van studenten.
...
Schrijf een functie die in een meegegeven string (dus als argument) van willekeurige lengte het aantal woorden telt. De woorden in de string worden gescheiden door spaties. Vergeet niet de woorden aan het begin en einde van de string mee te tellen. Het getelde aantal woorden wordt als returnwaarde teruggegeven. Maak gebruik van pointers. Test de functie met een hoofdprogramma.
...
Schrijf een programma dat de regels in een tekstbestand inleest en op lengte sorteert. Het resultaat schrijf je naar het scherm en naar een uitvoerbestand. De namen van de tekstbestanden moeten als argumenten op de command-line*) meegegeven worden. Ga ervan uit dat maximaal 1000 woorden kunnen worden opgeslagen.
...
Het resulterende bestand kan er dus zo uitzien:
...
\draw[red,thick,rounded corners=5pt] (-4.75,-3.65) rectangle ++(1.5,0.5);
\end{tikzpicture}
